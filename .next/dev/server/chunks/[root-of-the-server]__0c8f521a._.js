...
"[project]/src/app/api/founders/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
async function GET(request) {
    try {
        ...
        // Check if admin
        const { data: currentFounderData } = await supabase.from('founders').select('role').eq('user_id', user.id).single();
        const currentFounder = currentFounderData;
        if (currentFounder?.role === 'admin') {
            // Admin can see all founders
            const { searchParams } = new URL(request.url);
            const limit = parseInt(searchParams.get('limit') || '50');
            const offset = parseInt(searchParams.get('offset') || '0');
            const { data: founders, error, count } = await supabase.from('founders').select('*, trust_scores(*), companies(*)', {
                count: 'exact'
            ...
            const { data: founder, error } = await supabase.from('founders').select('*, trust_scores(*), companies(*)').eq('user_id', user.id).single();
            if (error) {
                return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
                    error: error.message
                }, {
...
async function POST(request) {
    try {
        ...
        const { data: founder, error } = await supabase.from('founders').insert(insertData).select().single();
        if (error) {
            return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
                error: error.message
            }, {
...
async function PATCH(request) {
    try {
        ...
        if (body.founderId) {
            const { data: adminData } = await supabase.from('founders').select('role').eq('user_id', user.id).single();
            ...
            const { data: founder, error } = await supabase.from('founders').update(adminUpdate).eq('id', body.founderId).select().single();
            if (error) {
                return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
                    error: error.message
                }, {
        ...
        const { data: founder, error } = await supabase.from('founders').update(updateData).eq('user_id', user.id).select().single();
        if (error) {
            return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
                error: error.message
            }, {
...
//# sourceMappingURL=%5Broot-of-the-server%5D__0c8f521a._.js.map