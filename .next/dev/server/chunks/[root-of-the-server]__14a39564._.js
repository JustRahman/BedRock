...
"[project]/src/app/api/trust-score/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
async function GET(request) {
    try {
        ...
        if (founder.role === 'admin' && founderId) {
            const { data: trustScore, error } = await supabase.from('trust_scores').select('*').eq('founder_id', founderId).single();
            if (error) {
                return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
                    error: error.message
                }, {
        ...
        const { data: trustScore, error } = await supabase.from('trust_scores').select('*').eq('founder_id', founder.id).single();
...
//# sourceMappingURL=%5Broot-of-the-server%5D__14a39564._.js.map