...
async function POST(request) {
    ...
    const { allowed, resetIn } = (0, __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$rate$2d$limit$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["checkRateLimit"])(`trust-score-calc:${ip}`, 10, 60_000);
    if (!allowed) {
    ...
    try {
        ...
        if (body.oauthData?.github) {
            v2Input.github = body.oauthData.github;
        ...
        if (body.oauthData?.linkedin) {
            v2Input.linkedin = body.oauthData.linkedin;
        ...
        if (body.oauthData?.stripe) {
            ...
            v2Input.economicActivity.stripe = body.oauthData.stripe;
        }
        // Digital presence
...
async function PUT(request) {
    try {
        ...
        const { data: existingScore } = await supabase.from('trust_scores').select('id, status').eq('founder_id', founder.id).single();
        const previousStatus = existingScore?.status;
...
//# sourceMappingURL=%5Broot-of-the-server%5D__35ffc0db._.js.map