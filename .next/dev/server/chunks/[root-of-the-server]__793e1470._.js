...
"[project]/src/app/api/service-requests/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
async function GET(request) {
    try {
        ...
        if (founder.role === 'admin') {
            let query = supabase.from('service_requests').select('*, founders(full_name, email), companies(name, state)').order('created_at', {
                ascending: false
            });
            if (serviceType) {
        ...
        let query = supabase.from('service_requests').select('*').eq('founder_id', founder.id).order('created_at', {
            ascending: false
        });
        if (serviceType) {
            query = query.eq('service_type', serviceType);
...
async function POST(request) {
    try {
        ...
        const { data: companyData } = await supabase.from('companies').select('id').eq('founder_id', founder.id).single();
        // Check for duplicate active request of same type
        const { data: existing } = await supabase.from('service_requests').select('id').eq('founder_id', founder.id).eq('service_type', body.serviceType).in('status', [
            'requested',
            'in_progress'
        ]).limit(1);
        ...
        const { data: serviceRequest, error } = await supabase.from('service_requests').insert(insertData).select().single();
        if (error) return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
            error: error.message
        }, {
            status: 500
...
async function PATCH(request) {
    try {
        ...
        const targetId = founder.role === 'admin' && body.requestId ? body.requestId : body.id;
        let queryBuilder = supabase.from('service_requests').update(updateData).eq('id', targetId);
...
//# sourceMappingURL=%5Broot-of-the-server%5D__793e1470._.js.map