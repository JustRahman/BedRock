...
async function POST(request) {
    try {
        ...
        // GitHub — form-based = username only (no real profile data)
        if (body.codeHistory?.hasGithub) {
            v2Input.githubUsernameOnly = !body.codeHistory.githubConnected;
        }
        // LinkedIn — form-based = URL only
        if (body.professional?.hasLinkedin) {
...
async function PUT(request) {
    try {
        ...
        const status = result.score >= 95 ? 'elite' : statusMap[result.risk_level] || 'review_needed';
...
//# sourceMappingURL=%5Broot-of-the-server%5D__99f587fd._.js.map