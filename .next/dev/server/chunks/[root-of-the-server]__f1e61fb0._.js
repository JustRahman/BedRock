...
"[project]/src/app/api/compliance/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
async function GET(request) {
    try {
        ...
        if (founder.role === 'admin') {
            let query = supabase.from('compliance_deadlines').select('*, founders(full_name, email)').order('due_date', {
                ascending: true
            });
            if (founderId) {
        ...
        const { data: deadlines, error } = await supabase.from('compliance_deadlines').select('*').eq('founder_id', founder.id).order('due_date', {
            ascending: true
        });
        if (error) {
            return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
...
async function POST(request) {
    try {
        ...
        const { data: deadline, error } = await supabase.from('compliance_deadlines').insert(insertData).select().single();
        if (error) {
            return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
                error: error.message
            }, {
...
async function PATCH(request) {
    try {
        ...
        const baseQuery = supabase.from('compliance_deadlines').update(updateData).eq('id', body.deadlineId);
        // Regular users can only update their own deadlines
        const query = founder.role !== 'admin' ? baseQuery.eq('founder_id', founder.id) : baseQuery;
...
async function DELETE(request) {
    try {
        ...
        // Build query with type assertion
        const baseQuery = supabase.from('compliance_deadlines').delete().eq('id', deadlineId);
...
//# sourceMappingURL=%5Broot-of-the-server%5D__f1e61fb0._.js.map